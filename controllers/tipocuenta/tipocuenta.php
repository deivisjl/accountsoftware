...
function guardar($parametros = array(), $db){
	...
	$query = "INSERT into tipocuenta 
			values(NULL,'{$parametros['nombre']}','{$parametros['descripcion']}','{$fecha}')";
	...
	if ($result) {
		...
		header("Location: $baseurl/tipocuenta.php");
...
function editar($parametros = array(), $db){
	...
	$query = "UPDATE tipocuenta 
				set nombre = '{$parametros['nombre']}', descripcion = '{$parametros['descripcion']}' 
					Where id = '{$id}'";
